# -*- coding: utf-8 -*-
"""
Created on Sat Dec 11 15:44:42 2021
...
# use this library for figure factory subplots visualization
import plotly.figure_factory as ff

# use this library for matplotlib and pyplot visualization
...
# In this example, we used the Newyork Times API in Orange to collect news articles,youtube subtitles and coinmarketcap news website data about cryptocurrency
@st.experimental_memo()
def load_corpus():
	return pd.read_csv("jet2.csv")
...
def show_sbs(PREVALENCE, DIVERSITY, CONNECTIVITY, SBS):
	...
	st.write(SBS)
#Method to split key and value
def convert_df_key(SBS):
   
    keys = SBS.keys()
    return keys

def convert_df_values(SBS):
  
    values = SBS.values()
    return values

#Method to create bar graph
def df_bargraph(df_con_key,df_con_value,title_label,ylabel):
    
    fig = plt.figure(figsize = (10, 5))

    plt.bar(df_con_key, df_con_value)
...
st.title("Semantic Brand Score for different cryptocurrencies")
...
st.markdown("""
	Calculating and comparing the Semantic Band Score (SBS) of Bitcoin, Ethereum, Cardano, Litecoin, Stellar and Ripple, the six largest cryptocurrencies by market capitalization, is the aim of this project.
	Computing the Semantic Brand Score of different cryptocurrencies involves the following steps:
	""")
...
st.markdown("""
	We get our corpus of documents from Newyork Times News, Coinmarketcap website and YouTube subtitles. 
	""")
corpus = load_corpus()
st.write(corpus)
...
brands = ["modularity","complexity","coordination"]
...
max_words = st.number_input("Keep most frequent words", min_value=0,max_value=250, value=90)
...
st.write("Documents", documents[:20])

st.header("Step 5: Link filtering")

st.markdown("Filter network links based on their weight.")

min_link_weight = st.slider("Minimum link weight", min_value=100, max_value=150, value=90)
...
#Split key and value
df_con_key=convert_df_key(sbs)
df_con_value=convert_df_values(sbs)
df_bargraph(df_con_key,df_con_value,"SEMANTIC BRAND SCORE VISUALIZATION","SBS")


df_con_key1=convert_df_key(prevalence)
df_con_value1=convert_df_values(prevalence)
df_bargraph(df_con_key1,df_con_value1,"PREVALENCE VISUALIZATION","PREVALENCE SCORE")

df_con_key2=convert_df_key(diversity)
df_con_value2=convert_df_values(diversity)
df_bargraph(df_con_key2,df_con_value2,"DIVERSITY VISUALIZATION","DIVERSITY SCORE")

df_con_key3=convert_df_key(connectivity)
df_con_value3=convert_df_values(connectivity)
df_bargraph(df_con_key3,df_con_value3,"CONNECTIVITY VISUALIZATION","CONNECTIVITY SCORE")
